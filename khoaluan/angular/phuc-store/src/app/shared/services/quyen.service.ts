...
export class QuyenService {
    private API: string = environment.api_url + '/api/quyen';
    public itemsSub: BehaviorSubject<Quyen[]>;
    public itemsObs: Observable<Quyen[]>;
    public isLoadingSub: BehaviorSubject<boolean>;
    public isLoadingObs: Observable<boolean>;
    public itemSub: BehaviorSubject<Quyen>;
    public itemObs: Observable<Quyen>;
    constructor(
        public http: HttpClient,
        private thongbaoService: ThongbaoService
    ) {
        this.itemsSub = new BehaviorSubject<Quyen[]>([]);
        this.itemsObs = this.itemsSub.asObservable();
        this.itemSub = new BehaviorSubject<Quyen>(null);
    ...
    referById(id: number) {
        ...
        this.http.get<Quyen>(url);
    }
    getAll() {
        this.isLoadingSub.next(true);
        return this.http.get<Quyen[]>(this.API).subscribe(
            res => {
                if (res['status'] === 'OK') {
    ...
    createNew(values: any) {
        this.isLoadingSub.next(true);
        this.http.post<Quyen>(this.API, values).subscribe(
            res => {
                if (res['status'] === 'OK') {
    ...
    update(value) {
        ...
        this.isLoadingSub.next(true);
        this.http.post<Quyen>(url, value).subscribe(
            res => {
                if (res['status'] === 'OK') {
