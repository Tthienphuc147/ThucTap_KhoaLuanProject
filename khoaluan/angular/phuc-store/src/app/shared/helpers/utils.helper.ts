...
export default class Utils {
  ...
  static viewImageUrl(id: string): string {
    return `${environment.api_img}api/file/${id}`;
  }

  static formatCodeUser(data: string, codeTemplate: string, specialCharacters: string[]) {
    for (var position = 0; position < codeTemplate.length - 1; position++) {
        if (specialCharacters.includes(codeTemplate[position]) && data[position] !== codeTemplate[position]) {
          data = [data.slice(0, position), codeTemplate[position], data.slice(position)].join('');
        }
    }
    return data;
  }
