...
export class NhaCungCapService {
    ...
    private API: string = environment.api_url + '/api/nhacungcap';
    constructor(
    ...
    referById(id: number) {
        ...
        this.http.get<NhaCungCap>(url);
    }
    getAll() {
        this.isLoadingSub.next(true);
        return this.http.get<NhaCungCap[]>(this.API).subscribe(
            res => {
                if (res['status'] === 'OK') {
    ...
    update(value) {
        ...
        this.isLoadingSub.next(true);
        this.http.post<NhaCungCap>(url, value).subscribe(
            res => {
                if (res['status'] === 'OK') {
